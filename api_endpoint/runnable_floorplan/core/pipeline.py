...
from ..detectors.openings_old import detect_doors_windows
...
log = logging.getLogger("four_wall_pipeline")
...
def build_artifacts(job_id: str) -> PipelineArtifacts:
    debug_dir = C.OUTPUT_DIR / f"{job_id}_debug"
...
def _estimate_wall_section(image_input: WallImageInput) -> WallSection:
    ...
    return WallSection(
        ...
        width_px=width_px,
        height_px=height_px,
        estimated_length_units=estimated_length_units,
        openings=openings,
        furniture=furniture,
...
def run_four_wall_pipeline(
    ...
    for item in wall_inputs:
        sections[item.wall] = _estimate_wall_section(item)
