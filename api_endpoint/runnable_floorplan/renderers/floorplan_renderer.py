...
def render_floorplan_preview(
    ...
    def to_px(x, y):
        ...
        py = image_size - (margin + (y - min_y) * scale)
        return (px, py)

    img = Image.new("RGB", (image_size, image_size), "white")
    draw = ImageDraw.Draw(img)

    polygon_px = [to_px(x, y) for x, y in pts]

    wall_px = max(2, int(wall_thickness * scale))
    draw.polygon(polygon_px, outline="black", width=wall_px)

    for opening in mapped_openings:
        wall_i = opening.wall_i
        p1 = pts[wall_i]
        p2 = pts[(wall_i + 1) % len(pts)]

        ox = p1[0] + (p2[0] - p1[0]) * float(opening.frac)
        oy = p1[1] + (p2[1] - p1[1]) * float(opening.frac)

        opx, opy = to_px(ox, oy)
        color = "red" if opening.type == "door" else "blue"
        r = 8
        draw.ellipse((opx - r, opy - r, opx + r, opy + r), fill=color, outline=color)

    for furn in mapped_furniture:
        fx, fy = furn.center
        fpx, fpy = to_px(float(fx), float(fy))

        fw = max(10, int(float(furn.size[0]) * scale))
        fh = max(10, int(float(furn.size[1]) * scale))
...
def render_floorplan_from_wall_sections(layout, out_png: Path, out_dxf: Path, wall_thickness: float, dxf_scale: float):
    pts_m = layout.pts_m
    pad = 0.95
    xlim = (pts_m[:, 0].min() - pad, pts_m[:, 0].max() + pad)
    ylim = (pts_m[:, 1].min() - pad, pts_m[:, 1].max() + pad)
    ...
    wall_segments = {
        "front": (pts_m[0], pts_m[1]),
        "right": (pts_m[1], pts_m[2]),
        "back": (pts_m[2], pts_m[3]),
        "left": (pts_m[3], pts_m[0]),
    }

    wall_order = ["front", "right", "back", "left"]
    openings_by_wall = defaultdict(list)
    room_center = np.asarray(layout.room_center, dtype=float)

    for wall_name, sec in layout.wall_sections.items():
        p1, p2 = wall_segments[wall_name]
        wall_vec = p2 - p1
        wall_len = np.linalg.norm(wall_vec)
        if wall_len < 1e-6:
            continue
        ...
        wall_mid = (p1 + p2) / 2.0
        if np.dot(normal, room_center - wall_mid) < 0:
            normal = -normal

        for op in sec.openings:
            op.width = float(op.width_m or 0.9)
            op.wall_len = float(wall_len)
            op.wall_dir = wall_dir
            op.normal = normal
            op.p1 = p1
            op.p2 = p2
            op.wall_i = wall_order.index(wall_name)
            op.center = p1 + float(op.frac) * wall_vec
            openings_by_wall[op.wall_i].append(op)

    for wall_i in range(4):
        draw_wall_segment(
            ax,
            pts_m[wall_i],
            pts_m[(wall_i + 1) % 4],
            openings_by_wall[wall_i],
            wall_thickness,
        )
    ...
    for wall_name, sec in layout.wall_sections.items():
        ...
        for furn in sec.furniture:
            frac = float(np.clip(furn.frac, 0.1, 0.9))
            wall_point = p1 + frac * wall_vec

            furn.angle = float(np.arctan2(wall_vec[1], wall_vec[0]))
            furn.size = furn.size_m if furn.size_m is not None else (0.60, 0.50)

            width = float(furn.size[0])
            depth = float(furn.size[1])
            gap = max(wall_thickness * 0.5, 0.08)

            center = wall_point + normal * (depth / 2.0 + gap)

            furn.center = center
            furn.wall_i = wall_order.index(wall_name)

            draw_furniture(ax, furn)
            mapped_furniture.append(furn)
    ...
    for wall_i in range(4):
        for op in openings_by_wall[wall_i]:
            if op.type == "door":
                draw_door(ax, op)
            else:
                draw_window(ax, op, wall_thickness)
    ...
    for i in range(4):
        ...
        p2 = pts_m[(i + 1) % 4]
        wall_len = np.linalg.norm(p2 - p1)
        draw_dimension(ax, p1, p2, room_center, f"{wall_len:.2f} m")
    ...
    ax.text(
        ...
        color="#2b2b2b",
        zorder=6,
        bbox=dict(boxstyle="round,pad=0.30", fc=(1, 1, 1, 0.82), ec="none"),
    )
    ...
    mapped_openings = []
    for wall_i in range(4):
        mapped_openings.extend(openings_by_wall[wall_i])

    class LayoutForDXF:
        pass

    layout_for_dxf = LayoutForDXF()
    layout_for_dxf.pts_m = layout.pts_m
    layout_for_dxf.room_center = room_center
